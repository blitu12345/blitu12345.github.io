...
1. Task was to extract information from NCERT textbook images and retrieve the page information for further use case.
2. Users were mostly indian school kids or parents doing self study.
3. Worked on line extraction from books image which sometimes were blurred, tilted, and curved using contour processing
4. Main hindrance to accurate system were blurred images(motion or focus), white noise or uneven lighting while capturing images.
5. Performed quality check using variance of laplacian and deblurring after removing gaussian noise(assumed blur noise to be gaussian)
6. OCR was used on all extracted lines from the images and saved text file for retrival.  
7. Evaluated tablet which was going to be used for our OCR system, such as focus, exposure, shutter time, frame quality etc and comapred performance of system on normal images and images from tablet.
8. Tech-stack: c++, Opencv, python, ocr, linux, windows
...
-> When tested on MNIST dataset my 2 layer network attained an accuracy of ~92%.
